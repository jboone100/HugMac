...
struct ChatModelTests {
    ...
    func make(_ workspace: Workspace, backend: FakeChatBackend = FakeChatBackend(),
        ...
        let name = "hugmac-chat-tests-\(UUID().uuidString)"
    ...
    @Test("Automatic picks the best installed model that runs well, and names a better one")
    func automaticPick() {
        let workspace = Workspace()
        defer { workspace.cleanUp() }
        ...
        chat.refresh()
        #expect(chat.current?.spec.repo == medium, "installing a better model upgrades Automatic by itself")
    }
