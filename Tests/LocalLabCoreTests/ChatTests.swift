...
struct ChatPickerTests {
    ...
    @Test func smartFitPrefersAModelThatRunsWellOverOneThatBarelyFits() {
        let picker = ChatModelPicker(hardware: m2Max, calibration: CalibrationStore())
        let choice = picker.smartFit(installed: [spec("9B").repo, spec("27B").repo])
        #expect(choice?.spec == spec("9B"))
    }

    @Test func smartFitTakesTheBestOfSeveralGoodOnes() {
        ...
        let choice = picker.smartFit(installed: [spec("4B").repo, spec("9B").repo, spec("27B").repo])
        #expect(choice?.spec == spec("27B"))
    }

    @Test func nothingInstalledMeansNothingSmartFit() {
        let picker = ChatModelPicker(hardware: m2Max, calibration: CalibrationStore())
        #expect(picker.smartFit(installed: []) == nil)
    }
