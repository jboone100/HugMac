...
func temporaryStore() -> ModelStore {
    let url = FileManager.default.temporaryDirectory
        .appendingPathComponent("locallab-tests-\(UUID().uuidString)", isDirectory: true)
...
struct OutputNameTests {
    ...
    func uniqueNames() throws {
        let directory = FileManager.default.temporaryDirectory
            .appendingPathComponent("locallab-names-\(UUID().uuidString)", isDirectory: true)
...
@Suite("Rename from HugMac")
struct LegacyMigrationTests {
    @Test func movesTheOldLibraryOnceAndLeavesALink() throws {
        let base = FileManager.default.temporaryDirectory
            .appendingPathComponent("locallab-rename-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: base) }
        let old = base.appendingPathComponent("HugMac/models/x", isDirectory: true)
        try FileManager.default.createDirectory(at: old, withIntermediateDirectories: true)
        try Data("w".utf8).write(to: old.appendingPathComponent("weights.safetensors"))

        #expect(LegacyMigration.moveApplicationSupportIfNeeded(appSupport: base))
        let moved = base.appendingPathComponent("LocalLab/models/x/weights.safetensors")
        #expect(FileManager.default.fileExists(atPath: moved.path))
        // An absolute path saved under the old name still resolves.
        #expect(FileManager.default.fileExists(atPath: base.appendingPathComponent("HugMac/models/x/weights.safetensors").path))
        #expect(!LegacyMigration.moveApplicationSupportIfNeeded(appSupport: base), "only once")
    }

    @Test func neverOverwritesAnExistingLibrary() throws {
        let base = FileManager.default.temporaryDirectory
            .appendingPathComponent("locallab-rename-\(UUID().uuidString)", isDirectory: true)
        defer { try? FileManager.default.removeItem(at: base) }
        try FileManager.default.createDirectory(at: base.appendingPathComponent("HugMac"), withIntermediateDirectories: true)
        try FileManager.default.createDirectory(at: base.appendingPathComponent("LocalLab"), withIntermediateDirectories: true)
        #expect(!LegacyMigration.moveApplicationSupportIfNeeded(appSupport: base))
    }
}
