...
private struct ModelBar: View {
            ...
            HStack(spacing: 10) {
                Menu {
                    Button {
                        ...
                        Text((model.selection == .automatic ? "✓ " : "") + "Automatic — best for this Mac")
                    }
                    Divider()
                ...
                if let fit = model.current {
                    ...
                    Menu("Context \(ChatModelPicker.contextLabel(fit.context))") {
                        Button("Automatic") { model.setContext(nil) }
    ...
    private var title: String {
        ...
        return model.selection == .automatic ? "Automatic — \(name)" : name
    }
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                if model.recommendations.isEmpty {
                    Text("No chat model in HugMac's list runs comfortably on this Mac. The smallest ones are in the model menu above, graded.")
                        .foregroundStyle(.secondary)
                }
            }
            .padding(20)
