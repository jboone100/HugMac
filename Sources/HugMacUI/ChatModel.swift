...
public final class ChatModel {
    ...
    static let selectionKey = "HugMac.chatModel"
    static let thinkingKey = "HugMac.chatThinking"
    static let renderModesKey = "HugMac.chatRenderModes"
    ...
    public init(
        ...
        selection = saved.map { .manual(repo: $0) } ?? .automatic
    ...
    /// change of selection. Automatic picks up a better installed model by itself.
    public func refresh() {
        ...
        switch selection {
        case .automatic:
            current = picker.automatic(installed: installed).map { fit in
    ...
    public func select(_ selection: Selection) {
        ...
        contextOverride = nil
        switch selection {
        case .automatic: defaults.removeObject(forKey: Self.selectionKey)
